...
| S9 | Ingest re-review follow-up | _Open_ — `IngestService` tests (deleted row mid-ingest, partial reprocess); delete orphaned R2 variants when `markReady` → `NOT_FOUND`; tighten concurrent `complete` on `pending`; avoid `markFailed` masking original error. _Nits:_ align presign TTL with `stale-pending` grace; admin review media `Cache-Control: private`; default MIME in `browser-upload`; drop unused `listPublishedForPublic` if still dead. |
