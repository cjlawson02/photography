...
- [x] D1 writes for portfolio domain metadata (ids, status, pending/ready/failed) — exact columns `_TBD_` in HLD *(provisional: `id`/`status`/`content_type`/timestamps; R2 keys derived from `id`; review photos same pattern)*
