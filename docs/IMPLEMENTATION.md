...
| T1 | Test suite | **Partial** — node:test for Workers/lib (`src/**/*.test.ts`); Vitest + RTL for admin UI (`src/**/*.vitest.{ts,tsx}` via `vitest.config.ts`); `npm test` runs both. Further migration of node tests `_TBD_` |
